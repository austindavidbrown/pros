...
VectorXd proximal_gradient_cd(VectorXd B, const MatrixXd& X, const VectorXd& y, const Vector7d& alpha, const double lambda) {
  ...
  for (int j = 0; j < max_iter; j++) {
    ...
    // Stop if Moreau-Yoshida gradient norm is less than tolerance
...
MatrixXd warm_start_proximal_gradient_cd(const MatrixXd& X, const VectorXd& y, const Vector7d& alpha, vector<double> lambdas) {
  int p = X.cols();
  int L = lambdas.size();
  ...
  B_matrix.row(0) = proximal_gradient_cd(B_0, X, y, alpha, lambdas[0]);

  // Warm start after the first one
  for (int l = 1; l < L; l++) {
    VectorXd B_warm = B_matrix.row((l - 1)); // warm start
    B_matrix.row(l) = proximal_gradient_cd(B_warm, X, y, alpha, lambdas[l]);
  }
...
CVType cross_validation_proximal_gradient_cd(const MatrixXd& X, const VectorXd& y, const double K, const Vector7d& alpha, vector<double> lambdas) {
  int n = X.rows();
  int p = X.cols();
  int L = lambdas.size();
  ...
  for (size_t k = 0; k < partitions.size(); k++) {
    ...
    MatrixXd B_matrix = warm_start_proximal_gradient_cd(X_train, y_train, alpha, lambdas);
    for (int l = 0; l < L; l++) {
