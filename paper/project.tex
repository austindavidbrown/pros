...
All of the variables are selected in each of the penalizations for the manually tuned scenario.
...
One source of confusion was noticing that \textbf{glmnet} uses a transformation for their $\lambda$ values and these do not correspond with the typical Lasso model. Thus, direct comparisons are complicated. Further adding to the complication is that \textbf{glmnet} provides varying results for fixed random seeds.
