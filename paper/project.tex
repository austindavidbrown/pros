...
\Repeat{Until Proximal Gradient Descent convergences}{
$h \gets \frac{1}{2}h$

$\beta \gets$ by Proximal Gradient Descent with step size $h$
}
...
\subsection{CRAN Issues}

At the time of this paper, the package in its current form will not be accepted onto the Comprehensive R Archive Network (CRAN).
There are some minor changes needed to comply with the CRAN policies and one larger issue.
In order to get good performance out of C++, the compiler needs to be aggressive. It seems that R allows many options for Fortran, but some modern options for C++ possibly need to be added. These options need to be available in general so the author will reach out to the R developers about this.
