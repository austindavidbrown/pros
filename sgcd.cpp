...
VectorXd predict(const MatrixXd X, const VectorXd B, const double intercept) {
  int n = X.rows();
  return intercept * VectorXd::Ones(n) + (X * B);
}

VectorXd sparsify(VectorXd B) {
  double tolerance = .01f;
...
// TODO I think there is an error with the lambdas being ordered
// We only need to order them once.
CVType cross_validation(MatrixXd X, VectorXd y, const double K, vector<double> lambdas) {
  ...
  for (size_t k = 0; k < partitions.size(); k++) {
    ...
    for (int l = 0; l < L; l++) {
      ...
      test_risks_matrix(l, k) = mean_squared_error(y_test, predict(X_test, B, intercept));
    }
  }
...
void test() {
  ...
  */
  ...
  VectorXd B = sparsify(sgcd(B_0, X_train, y_train, lambda));
  cout << mean_squared_error(y_train, predict(X_train, B, intercept));
  */
}
