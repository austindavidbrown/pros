...
def subgrad_coord_descent(X_, y_, lambda_):
  ...
  for j in range(0, max_iter):
    ...
    B_old = np.copy(B) # keep track of current B for stopping criterion
    for i in I:
      cX_i = cX[:, i]
      D_Li = -1 * cX_i.T @ (cy - (cX @ B_old))
      
      # handle subgradient cases of l1
      sub_grad_l1 = np.sign(B_old[i])
      if sub_grad_l1 != 0:
        B[i] = B_old[i] - s * (D_Li + lambda_ * sub_grad_l1)
      elif sub_grad_l1 == 0:
        if D_Li < -lambda_:
